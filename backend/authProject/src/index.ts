...
let passwordSchema = z.string().min(5, { message: "Must be 5 or more characters long" })
...
app.post("/api/signup", userMiddleware, async function (req: Request, res: Response) {
...
app.get("/api/signin", userMiddleware, async function (req: Request, res: Response) {
  let username = req.headers["username"] as string;
  let password = req.headers["password"] as string;
  try {
    let result = await prisma.user.findFirst({
